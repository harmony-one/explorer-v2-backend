    ...
    if (type === 'erc20' || type === 'erc721') {
      if (type === 'erc20') {
        txs = await this.query(
          `
            ...
            from (
                 (select * from contract_events ce where ce.from = $1 and ce.transaction_type = $2 and ce.event_type = 'Transfer' order by block_number desc limit $5)
                 union all
                 (select * from contract_events ce where ce.to = $1 and ce.transaction_type = $2 and ce.event_type = 'Transfer' order by block_number desc limit $5)
            ) ce
        ...
        txs = await this.query(
          `
            ...
            from (
                 (select * from contract_events ce where ce.from = $1 and ce.transaction_type = $2 and ce.event_type = 'Transfer' order by block_number desc limit $6)
                 union all
                 (select * from contract_events ce where ce.to = $1 and ce.transaction_type = $2 and ce.event_type = 'Transfer' order by block_number desc limit $6)
                 union all
                 (select * from contract_events ce where ce.from = $1 and ce.transaction_type = $3 and ce.event_type = 'Transfer' order by block_number desc limit $6)
                 union all
                 (select * from contract_events ce where ce.to = $1 and ce.transaction_type = $3 and ce.event_type = 'Transfer' order by block_number desc limit $6)
            ) ce
            join transactions t on t.hash = ce.transaction_hash
            order by ce.block_number desc
            offset $4
      ...
      txs = await Promise.all(
        txs.map(fromSnakeToCamelResponse).map(async (tx: any) => {
          tx.logs = await this.query('select * from logs where transaction_hash=$1', [
            tx.transactionHash,
          ])
          return tx
        })
    ...
    if (type === 'erc20') {
      const [{count}] = await this.query(
        `
          ...
          (
          select * from (
               (select * from contract_events ce where ce.from = $1 and ce.transaction_type = $2 and ce.event_type = 'Transfer' order by ce.block_number desc)
               union all
               (select * from contract_events ce where ce.to = $1 and ce.transaction_type = $2 and ce.event_type = 'Transfer' order by ce.block_number desc)
          ) ce
          limit $3
          ) ce2
